...
# %%
    ...
    DataDrivenFeatures, DataDrivenFeatures_Test = DataDrivenFeatures.iloc[
        train_index], DataDrivenFeatures.iloc[test_index]
...
# %%
    ...
    for cvturn in range(cvtimes):
        ...
        for train_index, val_index in kf.split(np.zeros(labels.shape[0]), labels.classNum):
            ...
            scalar_DataDrivenFeatures_train = (preprocessing.StandardScaler().fit(
                DataDrivenFeatures_train))
            DataDrivenFeatures_train = pd.DataFrame(scalar_DataDrivenFeatures_train.transform(
                DataDrivenFeatures_train), columns=DataDrivenFeatures_train.columns)
            DataDrivenFeatures_val = pd.DataFrame(scalar_DataDrivenFeatures_train.transform(
            ...
            score_DataDrivenFeatures = (getFeatureScore(
            ...
            for n_f in n_features:
                iniSlctFeat_train = pd.concat([GenericFeatures_train[selectByVote(score_GenericFeatures, n_f)], SemiGenericFeatures_train[selectByVote(score_SemiGenericFeatures, n_f)],
                                               DataDrivenFeatures_train[selectByVote(score_DataDrivenFeatures, n_f)], ModelBasedFeatures_train[selectByVote(score_ModelBasedFeatures, n_f)]], axis=1)
                iniSlctFeat_val = pd.concat([GenericFeatures_val[selectByVote(score_GenericFeatures, n_f)], SemiGenericFeatures_val[selectByVote(score_SemiGenericFeatures, n_f)],
                                            DataDrivenFeatures_val[selectByVote(score_DataDrivenFeatures, n_f)], ModelBasedFeatures_val[selectByVote(score_ModelBasedFeatures, n_f)]], axis=1)
                # classifier learning
                for n_est in n_estimators:
                    ...
                    valClassifResult.append(pd.DataFrame(
            ...
            fileName = 'tempSave_cvturn'+str(cvturn+1) + '_cvk'+str(k+1)+'.pkl'

            f = open(fileName, 'wb')
            pickle.dump([valClassifResult, CLFs, scalar_GenericFeatures_train,
                        scalar_SemiGenericFeatures_train, scalar_DataDrivenFeatures_train,
                        scalar_ModelBasedFeatures_train, score_GenericFeatures,
                        score_SemiGenericFeatures, score_DataDrivenFeatures,
                        score_ModelBasedFeatures, n_estimators, n_features,
                        val_index, cvk, cvtimes, methods, cvturn, fileName,
                        train_index, test_index, k, kf, n_est, n_f], f, -1)
            f.close()
    ...
    # valClassifResult, CLFs, scalar_GenericFeatures_train, scalar_SemiGenericFeatures_train, scalar_DataDrivenFeatures_train, scalar_ModelBasedFeatures_train, score_GenericFeatures, score_SemiGenericFeatures, score_DataDrivenFeatures, score_ModelBasedFeatures, n_estimators, n_features, val_index, cvk, cvtimes, methods, cvturn, fileName, train_index, test_index, k, kf, n_est, n_f = pickle.load(
    #     f)
